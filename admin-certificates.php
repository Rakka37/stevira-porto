...
// CSRF token
if (empty($_SESSION['csrf_cert'])) {
    $_SESSION['csrf_cert'] = bin2hex(random_bytes(16));
}
...
// --- certificates uploads dir (consistent with public file) ---
$certRel = 'uploads/';
$certSub = 'certificates/';
$certFsDir = __DIR__ . '/' . $certRel . $certSub;
if (!is_dir(__DIR__ . '/' . $certRel)) mkdir(__DIR__ . '/' . $certRel, 0755, true);
if (!is_dir($certFsDir)) mkdir($certFsDir, 0755, true);

$maxFileSize = 3 * 1024 * 1024;
$allowedExt = ['jpg','jpeg','png','gif','webp'];

// create certificates table if not exists (matches public)
$create_sql = "
...
function resolve_cert_image(array $r, string $certFsDir, string $certRel, string $certSub): string {
    $img = trim((string)($r['image'] ?? ''));
    if ($img === '') return '';
    $base = basename($img);
    $fs = $certFsDir . $base;
    if (file_exists($fs)) return $certRel . $certSub . rawurlencode($base);
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (!isset($_POST['csrf']) || !hash_equals($_SESSION['csrf_cert'], $_POST['csrf'])) {
        ...
        if ($action === 'add') {
            ...
            $saved = handle_upload($_FILES['image'] ?? [], $certFsDir, $allowedExt, $maxFileSize);
            $imgDb = $saved ?: null;

            $ins = $pdo->prepare("INSERT INTO certificates (title,image,description,date_text) VALUES (:title,:image,:description,:date_text)");
            $ins->execute([':title'=>$title, ':image'=>$imgDb, ':description'=>$desc, ':date_text'=>$date_text]);

            flash_set('success','Sertifikat ditambahkan.');
            $_SESSION['csrf_cert'] = bin2hex(random_bytes(16));
        ...
        if ($action === 'edit') {
            ...
            $cur = $pdo->prepare("SELECT * FROM certificates WHERE id=:id");
            $cur->execute([':id'=>$id]);
            $row = $cur->fetch(PDO::FETCH_ASSOC);
            if (!$row) throw new RuntimeException('Sertifikat tidak ditemukan.');

            $saved = handle_upload($_FILES['image'] ?? [], $certFsDir, $allowedExt, $maxFileSize);
            $imgDb = $saved ? $saved : $row['image'];

            if ($saved && !empty($row['image'])) {
                $old = $certFsDir . $row['image'];
                if (file_exists($old)) @unlink($old);
            }

            $upd = $pdo->prepare("UPDATE certificates SET title=:title,image=:image,description=:description,date_text=:date_text WHERE id=:id");
            $upd->execute([':title'=>$title, ':image'=>$imgDb, ':description'=>$desc, ':date_text'=>$date_text, ':id'=>$id]);
        ...
        if ($action === 'delete') {
            $id = (int)($_POST['id'] ?? 0);
            if ($id <= 0) throw new RuntimeException('ID tidak valid.');
            $cur = $pdo->prepare("SELECT image FROM certificates WHERE id=:id");
            $cur->execute([':id'=>$id]);
            $row = $cur->fetch(PDO::FETCH_ASSOC);
            if ($row && !empty($row['image'])) {
                $f = $certFsDir . $row['image'];
                if (file_exists($f)) @unlink($f);
            }
            $pdo->prepare("DELETE FROM certificates WHERE id=:id")->execute([':id'=>$id]);
            flash_set('success','Sertifikat dihapus.');
            $_SESSION['csrf_cert'] = bin2hex(random_bytes(16));
            header('Location: admin-certificates.php');
            exit;
        }
  ...
  <main class="main">
    ...
    <div class="card">
      <form method="post" enctype="multipart/form-data">
        <input type="hidden" name="csrf" value="<?=htmlspecialchars($_SESSION['csrf_cert'])?>">
        ...
        <div class="form-grid">
          ...
          <div class="field"><input type="text" name="title" placeholder="Judul Sertifikat" value="<?=htmlspecialchars($editItem['title'] ?? '')?>"></div>
          <div class="field"><input type="text" name="date_text" placeholder="hh/bb/tttt" value="<?=htmlspecialchars($editItem['date_text'] ?? '')?>"></div>
        ...
        <div style="margin-top:12px;text-align:center">
          <button type="submit" class="btn-save"><?= $editItem ? 'Simpan Perubahan' : 'Tambah Sertifikat' ?></button>
          <?php if ($editItem): ?><a class="btn-cancel" href="admin-certificates.php">Batal</a><?php endif; ?>
        </div>
      </form>
    </div>
      ...
      <table>
        ...
        <tbody>
          <?php if (empty($certificates)): ?>
            <tr><td colspan="5" style="padding:18px;background:#fff">Belum ada sertifikat.</td></tr>
          <?php else: ?>
            <?php foreach($certificates as $c):
              $img = resolve_cert_image($c, $certFsDir, $certRel, $certSub);
            ?>
            <tr>
              <td><img src="<?=htmlspecialchars($img ?: 'p-placeholder.png')?>" alt="" class="thumb"></td>
              <td><?=htmlspecialchars($c['title'])?></td>
              <td><?=nl2br(htmlspecialchars($c['description']))?></td>
              <td><?=htmlspecialchars($c['date_text'])?></td>
              <td>
                <a href="admin-certificates.php?edit=<?= (int)$c['id'] ?>" style="display:inline-block;padding:6px 10px;background:#f6d86c;border-radius:6px;margin-right:6px;text-decoration:none;color:#111;font-weight:700">Edit</a>
